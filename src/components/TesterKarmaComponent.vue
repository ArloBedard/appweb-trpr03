...
const handleGiveKarma = async () => {
    ...
    try {
        karma.value += 10
        await testerStore.modifyKarma(email.value, karma.value)
        if (onError.value) {
        ...
        <Form>
            <div class="mb-3">
                ...
                <Field as="select" class="form-select" id="email-input" name="email-input" type="email"
                    ...
                    <option v-for="tester in testers" :key="tester.id" :value="tester.email">
                        {{ tester.email }}
                    </option>
                </Field>
                <ErrorMessage class="text-danger" name="email-input" />
            </div>
            <button class="btn btn-success  m-2" name="give-karma" @click="handleGiveKarma">+ 10 karma</button>
            <button class="btn btn-danger m-2" name="take-karma" @click="handleTakeKarma">- 10 karma</button>
        </Form>
    </div>
</template>
